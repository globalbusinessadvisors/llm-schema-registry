...
async function install() {
  try {
    ...
    const binaryName = `llm-schema-api${ext}`;
    ...
    console.log(`Installing LLM Schema API v${VERSION} for ${platform}...`);
    ...
    try {
      ...
      console.log('You can build from source by running: cargo install llm-schema-api');

      // Check if cargo is available
      try {
        execSync('cargo --version', { stdio: 'ignore' });
        console.log('\n📦 Attempting to build from source with cargo...');
        execSync('cargo install --git https://github.com/globalbusinessadvisors/llm-schema-registry llm-schema-api', {
          stdio: 'inherit'
        });
        ...
        console.error('Then run: cargo install llm-schema-api');
        process.exit(1);
      }
    }
  } catch (error) {
