...
async function install() {
  try {
    ...
    const binaryName = `schema-registry-server${ext}`;
    ...
    console.log(`Installing LLM Schema Registry Server v${VERSION} for ${platform}...`);
    ...
    try {
      ...
      console.log('You can build from source by running: cargo install schema-registry-server');

      // Check if cargo is available
      try {
        execSync('cargo --version', { stdio: 'ignore' });
        console.log('\n📦 Attempting to build from source with cargo...');
        execSync('cargo install --git https://github.com/globalbusinessadvisors/llm-schema-registry schema-registry-server', {
          stdio: 'inherit'
        });
        ...
        console.error('Then run: cargo install schema-registry-server');
        process.exit(1);
      }
    }
  } catch (error) {
